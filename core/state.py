...
KV_DND_ON, KV_DND_SINCE = "dnd_on", "dnd_since"


async def load_dnd() -> None:
    global dnd_on, dnd_since
    dnd_on = bool(await db.kv_get(KV_DND_ON, ""))
    dnd_since = int(await db.kv_get(KV_DND_SINCE, "0") or 0)
    dnd_replied.clear()


async def set_dnd() -> None:
    global dnd_on, dnd_since
    dnd_on, dnd_since = True, int(time.time())
    dnd_replied.clear()
    await db.kv_set(KV_DND_ON, "1")
